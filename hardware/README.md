...
Editable KiCad 10 design for the ESP32-C5 GPIO controller. Open `saihub.kicad_pro`; symbols and footprints are project-local. The schematic is one A3 page with six functional sections.
...
**This revision is fully routed on two copper layers.** All 55 footprints remain on top in the approved placement. KiCad reports zero ERC violations, zero DRC violations, zero unconnected items and zero schematic parity issues. Both layers have filled ground copper; regulator/module ground vias and fixed critical power/USB routes are included. Connector pins and the switch actuator project beyond the outline; PCB dimensions are not the full assembled enclosure envelope.
...
- `docs/pcb.pdf`: top and mirrored bottom PCB previews.
...
The command synchronizes board metadata, requires zero ERC/DRC/parity findings and zero unconnected items, then refreshes the three files in `docs/`. It requires KiCad 10, ImageMagick and Python `reportlab` (`python3 -m pip install reportlab`). Set `KICAD_CLI` and `KICAD_PYTHON` to override the macOS tool defaults; the latter must provide `pcbnew`. Poppler is optional for the separate schematic PNG.
...
For regeneration and routing, read [the project workflow skill](agent/skills/kicad-workflow/SKILL.md). `generate.py` writes the separate `agent/routing/saihub-unrouted.kicad_pcb`; it preserves the main routed PCB but regenerates the schematic, libraries and project settings. Existing SES routing is tied to its exact placement and fixed routes.
