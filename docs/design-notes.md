...
4. After approval the device mints one 32-character `grantSecret`, stores a C struct array of `{createdAtUs, name, grantSecret}` as an NVS blob (`cloud.grants`), and sends a single unsolicited `pairingSessionTokenResponse`. The Durable Object stores that secret, broadcasts it to waiters, and serves later `/pairing/token` calls with the same secret until TTL. `GET /cloud/grant-secrets` returns `createdAt` in microseconds.
