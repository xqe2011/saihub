...
static ToolCall_Result ToolCall_ConfigurePins(cJSON* args)
{
  ...
  if (ToolCall_CheckPinsLock(&sel, LOCK_METHOD_WRITE, lockId, reason, sizeof(reason))) {
    return ToolCall_Err(reason);
  }
  for (size_t i = 0; i < sel.count; i++) {
    esp_err_t cfg = GpioCtrl_SetConfig(sel.pins[i], mode, openDrain, pullUp, pullDown);
    if (cfg == ESP_ERR_NO_MEM) {
      return ToolCall_Err("PWM resources exhausted. Free another pwmOutput pin or reuse an existing frequency.");
    }
    ...
    if (cfg != ESP_OK) return ToolCall_Err("internal");
  }
  Lock_Touch(lockId);
...
static ToolCall_Result ToolCall_GetPinLevels(cJSON* args)
{
  ...
  const char* lockId = ToolCall_GetLockId(args);
  if (ToolCall_CheckPinsLock(&sel, LOCK_METHOD_READ, lockId, reason, sizeof(reason))) {
    return ToolCall_Err(reason);
  }
  cJSON* root = cJSON_CreateObject();
...
static ToolCall_Result ToolCall_SetPinLevels(cJSON* args)
{
  ...
  for (size_t i = 0; i < sel.count; i++) {
    if (GpioCtrl_SetLevel(sel.pins[i], level) != ESP_OK) return ToolCall_Err("internal");
  }
  Lock_Touch(lockId);
  return ToolCall_OkEmpty();
}
...
static ToolCall_Result ToolCall_PulsePins(cJSON* args)
{
  ...
  for (size_t i = 0; i < sel.count; i++) {
    if (GpioCtrl_Pulse(sel.pins[i], level, width) != ESP_OK) return ToolCall_Err("internal");
  }
  Lock_Touch(lockId);
  return ToolCall_OkEmpty();
}
...
static ToolCall_Result ToolCall_SetPinPwms(cJSON* args)
{
  ...
  for (size_t i = 0; i < sel.count; i++) {
    ...
    if (pwm != ESP_OK) return ToolCall_Err("internal");
  }
  Lock_Touch(lockId);
  return ToolCall_OkEmpty();
}
...
static ToolCall_Result ToolCall_TracePins(cJSON* args)
{
  ...
  const char* lockId = ToolCall_GetLockId(args);
  if (ToolCall_CheckPinsLock(&sel, LOCK_METHOD_READ | LOCK_METHOD_WRITE, lockId, reason, sizeof(reason))) {
    return ToolCall_Err(reason);
  }
  GpioCtrl_TraceEvent* events = calloc(CONFIG_GPIO_TRACE_MAX_EVENTS, sizeof(GpioCtrl_TraceEvent));
  if (events == NULL) return ToolCall_Err("internal");
  size_t eventCount = 0;
...
static ToolCall_Result ToolCall_GetOutputPowerState(cJSON* args)
{
  ...
  const char* lockId = ToolCall_GetLockId(args);
  if (HttpServer_LockStatusId(lockId, ToolCall_PowerLockKind(rail), 0, LOCK_METHOD_READ, reason, sizeof(reason))) {
    return ToolCall_Err(reason);
  }
  bool enable = false;
  if (GpioCtrl_GetPowerEnable(rail, &enable) != ESP_OK) return ToolCall_Err("internal");
  cJSON* root = cJSON_CreateObject();
...
static ToolCall_Result ToolCall_SetOutputPowerState(cJSON* args)
{
  ...
  const char* lockId = ToolCall_GetLockId(args);
  if (HttpServer_LockStatusId(lockId, ToolCall_PowerLockKind(rail), 0, LOCK_METHOD_WRITE, reason, sizeof(reason))) {
    return ToolCall_Err(reason);
  }
  if (GpioCtrl_SetPowerEnable(rail, cJSON_IsTrue(enableItem)) != ESP_OK) return ToolCall_Err("internal");
  Lock_Touch(lockId);
  return ToolCall_OkEmpty();
}
...
static ToolCall_Result ToolCall_CreateLock(cJSON* args)
{
  ...
  if (cret != ESP_OK) return ToolCall_Err("internal");
