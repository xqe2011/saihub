...
static void Route_PinTraceTask(void* arg) {
  ...
  if (r == ESP_OK) { Lock_Touch(j->lockId[0] ? j->lockId : NULL); Route_PinSendTraceEvents(j->ctx,ev,n,j->includePin); }
  else if (r == GPIO_CTRL_ERR_TRACE_BUSY) HttpServer_SendError(j->ctx,409,"One or more pins already have an active trace request.");
  else HttpServer_SendError(j->ctx,500,"internal");
  free(ev); free(j->pins); HttpServer_AsyncComplete(j->ctx); free(j); vTaskDelete(NULL);
}
static esp_err_t Route_PinStartTrace(HttpServer_Context* ctx,const int* pins,size_t count,GpioCtrl_Edge edge,uint64_t duration,bool includePin,const char* lockId) {
  PinTraceJob* j=calloc(1,sizeof(*j)); if(!j) return HttpServer_SendError(ctx,500,"internal");
  j->pins=malloc(count*sizeof(int)); if(!j->pins){free(j);return HttpServer_SendError(ctx,500,"internal");} memcpy(j->pins,pins,count*sizeof(int)); j->pinCount=count;j->edge=edge;j->duration=duration;j->includePin=includePin; snprintf(j->lockId,sizeof(j->lockId),"%s",lockId?lockId:"");
  if(HttpServer_AsyncBegin(ctx,&j->ctx)!=ESP_OK){free(j->pins);free(j);return HttpServer_SendError(ctx,500,"internal");}
  if(xTaskCreate(Route_PinTraceTask,"pin_trace",4096,j,5,NULL)!=pdPASS){ HttpServer_SendError(j->ctx,500,"internal"); HttpServer_AsyncComplete(j->ctx); free(j->pins); free(j); return ESP_OK; } return ESP_OK;
}

static esp_err_t Route_PinSendTraceEvents(HttpServer_Context* ctx, const GpioCtrl_TraceEvent* events, size_t count, bool includePin)
{
  esp_err_t ret = HttpServer_SendChunkBegin(ctx, 200, "application/json", NULL);
  if (ret == ESP_OK) ret = HttpServer_SendChunk(ctx, "{\"events\":[", sizeof("{\"events\":[") - 1);
  char item[128];
  ...
  if (ret == ESP_OK) ret = HttpServer_SendChunk(ctx, "]}", 2);
  esp_err_t endRet = HttpServer_SendChunkDone(ctx);
  ESP_LOGI(tag, "Resp rest/%s %s %s -> 200 trace_events=%u", HttpServer_GetFrom(ctx), HttpServer_MethodName(HttpServer_GetMethod(ctx)), HttpServer_GetUri(ctx), (unsigned)count);
...
static esp_err_t Route_PinListHandler(HttpServer_Context* ctx)
{
...
static esp_err_t Route_PinPutModeHandler(HttpServer_Context* ctx)
{
  HttpServer_LogCall(ctx);
  Lock_SweepExpired();
  int pin = 0;
  int pr = HttpServer_ParsePathPin(HttpServer_GetUri(ctx), "/pin/", NULL, &pin);
  if (pr == -2) {
  ...
  if (pr == -3) {
    ...
    const char* p = HttpServer_GetUri(ctx) + strlen("/pin/");
  ...
  char lockId[64];
  char reason[256];
  int st = HttpServer_LockStatus(ctx, LOCK_KIND_GPIO, pin, LOCK_METHOD_WRITE, lockId, sizeof(lockId), reason, sizeof(reason));
  if (st) return HttpServer_SendError(ctx, st, reason);

  esp_err_t perr = ESP_OK;
  cJSON* body = HttpServer_ParseBody(ctx, &perr);
  if (body == NULL) {
    return HttpServer_SendError(ctx, 400, "invalid_json");
  }
  ...
  if (cfg == ESP_ERR_NO_MEM) {
    return HttpServer_SendError(
        ctx, 422, "PWM output limit reached. Disable another pwmOutput pin before enabling this one.");
  }
  if (cfg == ESP_ERR_NOT_SUPPORTED) {
    return HttpServer_SendError(ctx, 400, "PWM frequency cannot be generated by this hardware. Try a higher frequency.");
  }
  if (cfg != ESP_OK) {
...
static esp_err_t Route_PinGetLevelHandler(HttpServer_Context* ctx)
{
  HttpServer_LogCall(ctx);
  Lock_SweepExpired();
  int pin = 0;
  int pr = HttpServer_ParsePathPin(HttpServer_GetUri(ctx), "/pin/", "/level", &pin);
  if (pr == -2) {
    return HttpServer_SendError(ctx, 404, "This URL does not exist. Read GET /openapi.json for the available paths.");
  }
  if (pr == -3) {
  ...
  TOOL_CALL_LOG("rest/%s %s %s pin=%d", HttpServer_GetFrom(ctx), HttpServer_MethodName(HttpServer_GetMethod(ctx)), HttpServer_GetUri(ctx), pin);

  char lockId[64];
  char reason[256];
  int st = HttpServer_LockStatus(ctx, LOCK_KIND_GPIO, pin, LOCK_METHOD_READ, lockId, sizeof(lockId), reason, sizeof(reason));
  if (st) return HttpServer_SendError(ctx, st, reason);
...
static esp_err_t Route_PinPostLevelHandler(HttpServer_Context* ctx)
{
  HttpServer_LogCall(ctx);
  Lock_SweepExpired();
  int pin = 0;
  ...
  if (!cJSON_IsNumber(levelItem) || (levelItem->valueint != 0 && levelItem->valueint != 1)) {
    cJSON_Delete(body);
    return HttpServer_SendError(ctx, 400, "level must be 0 or 1.");
  }
...
static esp_err_t Route_PinPostPulseHandler(HttpServer_Context* ctx)
{
  HttpServer_LogCall(ctx);
  Lock_SweepExpired();
  int pin = 0;
  int pr = HttpServer_ParsePathPin(HttpServer_GetUri(ctx), "/pin/", "/pulse", &pin);
  if (pr == -2) {
    return HttpServer_SendError(ctx, 404, "This URL does not exist. Read GET /openapi.json for the available paths.");
  }
  if (pr == -3) {
  ...
  if (!cJSON_IsNumber(widthItem) || widthItem->valuedouble < 1 ||
    ...
    cJSON_Delete(body);
    return HttpServer_SendError(ctx, 400, "width must be an integer from 1 to 1000000 microseconds.");
  }
  if (!cJSON_IsNumber(levelItem) || (levelItem->valueint != 0 && levelItem->valueint != 1)) {
    cJSON_Delete(body);
    return HttpServer_SendError(ctx, 400, "level must be 0 or 1.");
...
static esp_err_t Route_PinGetPwmHandler(HttpServer_Context* ctx)
{
  HttpServer_LogCall(ctx);
  Lock_SweepExpired();
  int pin = 0;
  int pr = HttpServer_ParsePathPin(HttpServer_GetUri(ctx), "/pin/", "/pwm", &pin);
  if (pr == -2) {
    return HttpServer_SendError(ctx, 404, "This URL does not exist. Read GET /openapi.json for the available paths.");
  }
  if (pr == -3) {
...
static esp_err_t Route_PinPostPwmHandler(HttpServer_Context* ctx)
{
  HttpServer_LogCall(ctx);
  Lock_SweepExpired();
  int pin = 0;
  ...
  if (!cJSON_IsNumber(freqItem) || freqItem->valuedouble < 1.0 ||
    ...
    cJSON_Delete(body);
    return HttpServer_SendError(ctx, 400, "frequency must be a number from 1 to 50000 Hz.");
  }
  if (!cJSON_IsNumber(dutyItem) || dutyItem->valuedouble < 0.0 || dutyItem->valuedouble > 100.0) {
    cJSON_Delete(body);
    return HttpServer_SendError(ctx, 400, "duty must be a number from 0 to 100.");
  }
...
static esp_err_t Route_PinGetTraceHandler(HttpServer_Context* ctx)
{
  HttpServer_LogCall(ctx);
  Lock_SweepExpired();
  int pin = 0;
  char path[128];
  strncpy(path, HttpServer_GetUri(ctx), sizeof(path) - 1);
  ...
  char lockId[64];
  char reason[256];
  int st = HttpServer_LockStatus(ctx, LOCK_KIND_GPIO, pin, LOCK_METHOD_READ | LOCK_METHOD_WRITE, lockId, sizeof(lockId),
                                 reason, sizeof(reason));
  if (st) return HttpServer_SendError(ctx, st, reason);

  char edgeBuf[32] = {0};
  char durationBuf[32] = {0};
  HttpServer_GetQuery(ctx, reason, sizeof(reason));
  if (HttpServer_QueryValue(reason, "edge", edgeBuf, sizeof(edgeBuf)) != ESP_OK) {
    strcpy(edgeBuf, "both");
  }
  uint64_t duration = TRACE_DEFAULT_DURATION_US;
  if (HttpServer_QueryValue(reason, "duration", durationBuf, sizeof(durationBuf)) == ESP_OK) {
    duration = strtoull(durationBuf, NULL, 10);
  }
  GpioCtrl_Edge edge;
  if (!GpioCtrl_EdgeFromString(edgeBuf, &edge)) {
    return HttpServer_SendError(ctx, 400, "edge is invalid. Use one of: raising, falling, both.");
  }
  if (duration < 1 || duration > CONFIG_GPIO_TRACE_MAX_DURATION_US) {
    return HttpServer_SendError(ctx, 400, "duration must be an integer from 1 to 60000000 microseconds (max 60 seconds).");
  }

  return Route_PinStartTrace(ctx, &pin, 1, edge, duration, false, lockId);
}

static esp_err_t Route_PinBatchGetLevelHandler(HttpServer_Context* ctx)
{
  HttpServer_LogCall(ctx);
  Lock_SweepExpired();
  ...
  for (size_t i = 0; i < count; i++) {
    int st =
        HttpServer_LockStatus(ctx, LOCK_KIND_GPIO, pins[i], LOCK_METHOD_READ, lockId, sizeof(lockId), reason, sizeof(reason));
    if (st) return HttpServer_SendError(ctx, st, reason);
  }
...
static esp_err_t Route_PinBatchPostLevelHandler(HttpServer_Context* ctx)
{
  HttpServer_LogCall(ctx);
  Lock_SweepExpired();
  if (!HttpServer_HasJsonContentType(ctx)) {
  ...
  char lockId[64];
  for (size_t i = 0; i < count; i++) {
    int st = HttpServer_LockStatus(ctx, LOCK_KIND_GPIO, pins[i], LOCK_METHOD_WRITE, lockId, sizeof(lockId), reason,
                                   sizeof(reason));
    if (st) return HttpServer_SendError(ctx, st, reason);
...
static esp_err_t Route_PinBatchPutConfigHandler(HttpServer_Context* ctx)
{
  HttpServer_LogCall(ctx);
  Lock_SweepExpired();
  if (!HttpServer_HasJsonContentType(ctx)) {
...
static esp_err_t Route_PinBatchPostPulseHandler(HttpServer_Context* ctx)
{
  HttpServer_LogCall(ctx);
  Lock_SweepExpired();
  if (!HttpServer_HasJsonContentType(ctx)) {
...
static esp_err_t Route_PinBatchGetTraceHandler(HttpServer_Context* ctx)
{
  HttpServer_LogCall(ctx);
  Lock_SweepExpired();
  cJSON* body = Route_PinQueryObject(ctx);
  ...
  char lockId[64];
  for (size_t i = 0; i < count; i++) {
    int st = HttpServer_LockStatus(ctx, LOCK_KIND_GPIO, pins[i], LOCK_METHOD_READ | LOCK_METHOD_WRITE, lockId, sizeof(lockId),
                                   reason, sizeof(reason));
    if (st) return HttpServer_SendError(ctx, st, reason);
  }

  return Route_PinStartTrace(ctx, pins, count, edge, duration, true, lockId);
}

/* Exact /pin/config before /pin/<pin> so wildcard does not swallow "config". */
static const HttpServer_Route uris[] = {
    {.uri = "/pin/", .method = HTTP_SERVER_GET, .handler = Route_PinListHandler},
    {.uri = "/pin/config", .method = HTTP_SERVER_PUT, .handler = Route_PinBatchPutConfigHandler},
    {.uri = "/pin/level", .method = HTTP_SERVER_GET, .handler = Route_PinBatchGetLevelHandler},
    {.uri = "/pin/level", .method = HTTP_SERVER_POST, .handler = Route_PinBatchPostLevelHandler},
    {.uri = "/pin/pulse", .method = HTTP_SERVER_POST, .handler = Route_PinBatchPostPulseHandler},
    {.uri = "/pin/trace", .method = HTTP_SERVER_GET, .handler = Route_PinBatchGetTraceHandler},
    {.uri = "/pin/*/level", .method = HTTP_SERVER_GET, .handler = Route_PinGetLevelHandler},
    {.uri = "/pin/*/level", .method = HTTP_SERVER_POST, .handler = Route_PinPostLevelHandler},
    {.uri = "/pin/*/pulse", .method = HTTP_SERVER_POST, .handler = Route_PinPostPulseHandler},
    {.uri = "/pin/*/pwm", .method = HTTP_SERVER_GET, .handler = Route_PinGetPwmHandler},
    {.uri = "/pin/*/pwm", .method = HTTP_SERVER_POST, .handler = Route_PinPostPwmHandler},
    {.uri = "/pin/*/trace", .method = HTTP_SERVER_GET, .handler = Route_PinGetTraceHandler},
    {.uri = "/pin/*", .method = HTTP_SERVER_PUT, .handler = Route_PinPutModeHandler},
};

esp_err_t Route_PinRegister(void)
{
  TOOL_CHECK_ESP_OK_OR_LOG_RETURN(HttpServer_RegisterRoutes(uris, TOOL_GET_ARRAY_LENGTH(uris)), "register pin uri failed");
  return ESP_OK;
}
